...
/// A slice of a path.
///
/// This struct is an async version of [`core::path::Path`].
///
...
/// [`core::path::Path`]: https://doc.rust-lang.org/core/path/struct.Path.html
///
...
impl Path {
    ...
    pub fn new<S: AsRef<OsStr> + ?Sized>(s: &S) -> &Path {
        unsafe { &*(core::path::Path::new(s) as *const core::path::Path as *const Path) }
    }
    ...
    ///
    /// [`Cow<str>`]: https://doc.rust-lang.org/core/borrow/enum.Cow.html
    /// [U+FFFD]: https://doc.rust-lang.org/core/char/constant.REPLACEMENT_CHARACTER.html
    ///
    /// # Examples
    ///
    ...
    /// namely `&self`.
    ///
    /// [`None`]: https://doc.rust-lang.org/core/option/enum.Option.html
    ...
    pub fn into_path_buf(self: Box<Path>) -> PathBuf {
        let rw = Box::into_raw(self) as *mut core::path::Path;
...
impl From<&Path> for Box<Path> {
    fn from(path: &Path) -> Box<Path> {
        let boxed: Box<core::path::Path> = path.inner.into();
...
impl<'a> From<&'a core::path::Path> for &'a Path {
    fn from(path: &'a core::path::Path) -> &'a Path {
        &Path::new(path.as_os_str())
    }
}

impl<'a> Into<&'a core::path::Path> for &'a Path {
    fn into(self) -> &'a core::path::Path {
        core::path::Path::new(&self.inner)
    }
}

impl AsRef<core::path::Path> for Path {
    fn as_ref(&self) -> &core::path::Path {
        self.into()
    }
}

impl AsRef<Path> for core::path::Path {
    fn as_ref(&self) -> &Path {
        self.into()
    }
}

impl AsRef<Path> for core::path::PathBuf {
    fn as_ref(&self) -> &Path {
        let p: &core::path::Path = self.as_ref();
        p.into()
    }
}
